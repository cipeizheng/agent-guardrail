...
| `pii` | `pii` | Invariant 英文 Presidio 基线加本地校验规则；可选 NER backend 由部署方注入 |
...
| `embedding_similarity` / `is_similar` | `is_similar` Similarity condition | string/list、max-pair 和命名阈值对齐；部署 profile 选择 encoder model |
